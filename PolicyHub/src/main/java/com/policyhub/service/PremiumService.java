...
public class PremiumService {
    @Autowired private PremiumRepository premiumRepository;
    @Autowired private CustomerRepository customerRepository;
    @Autowired private PolicyRepository policyRepository;

    // Central calculation used by policy creation and when buying
    public BigDecimal calculatePremium(Policy policy) {
        double baseRate;
        String pt = policy.getPolicyType() == null ? "" : policy.getPolicyType().toLowerCase();
        switch (pt) {
            case "life insurance": baseRate = 0.8; break;
            case "health insurance": baseRate = 1.0; break;
            case "car insurance": baseRate = 1.2; break;
            case "home insurance": baseRate = 0.6; break;
            case "travel insurance": baseRate = 1.5; break;
    ...
    public List<Premium> getAllPremiums() { return premiumRepository.findAll(); }
}
